...
function validate(dir){
  ...
  for(const rel of ['references/design-rules.md','references/diagram-types.md','references/validation.md','references/review-protocol.md','references/review-ui.md','assets/review-canvas.html'])if(!fs.existsSync(path.join(skill,rel)))errs.push(`missing ${rel}`);
  ...
  for(const marker of ['VA:DIAGRAM_START','VA:DIAGRAM_END','data-va-id','id="va-meta"'])if(canvas&&!canvas.includes(marker))errs.push(`Review Studio missing ${marker}`);
...
case 'doctor':{
  ...
  out(`Visual Architect: ${pkg.version}`);out(`Node: ${process.version} ${major>=18?'✓':'✖ requires >=18'}`);out(`Platform: ${process.platform} ${process.arch}`);out(`Skill bundle: ${fs.existsSync(path.join(skillSrc,'SKILL.md'))?'✓':'✖'}`);out(`Review Studio: ${fs.existsSync(path.join(skillSrc,'assets','review-canvas.html'))?'✓':'✖'}`);out(`Showcase: ${fs.existsSync(path.join(skillSrc,'examples','school-erp-fee-payment.html'))?'✓':'✖'}`);if(major<18)process.exitCode=1;break;}
case 'init':{
